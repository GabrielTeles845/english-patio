...
section('Agenda: salas, turmas, mover aluno e fila');
{
  ...
  W.setAgView('mural');
  const paginas = $('agBody').querySelectorAll('.bg-white').length;
  paginas >= 5 ? ok(`mural mostra as páginas das salas lado a lado (${paginas})`) : fail(`mural com só ${paginas} páginas`);
  $('agBody').textContent.includes('NÃO TEM VAGA') ? ok('mural mantém o formato do Canva (NÃO TEM VAGA)') : fail('mural sem o vocabulário do Canva');
  W.setAgView('niveis');
