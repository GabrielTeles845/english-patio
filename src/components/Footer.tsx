import { PhoneIcon, EnvelopeIcon, MapPinIcon } from '@heroicons/react/24/outline';
import { c } from '../content/site';

const Footer = () => {
  const basePath = '';
  
  return (
    <footer className="bg-slate-50">
      {/* Seção principal do footer */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          {/* Logo e descrição */}
          <div className="col-span-1 md:col-span-2">
            <img src="/assets/logo-index.webp" alt="English Patio" className="h-12 w-auto mb-4" />
            <p className="text-gray-600 mb-4">
              {c('global', 'footer-descricao-1')}
            </p>
            <p className="text-gray-600 mb-4">
              {c('global', 'footer-descricao-2')}
            </p>
            <p className="text-gray-600">
              {c('global', 'footer-descricao-3')}
            </p>
          </div>

          {/* Links rápidos */}
          <div>
            <h3 className="text-primary font-semibold mb-4">{c('global', 'footer-links-titulo')}</h3>
            <ul className="space-y-3">
              <li>
                <a href={`${basePath}/`} className="text-gray-600 hover:text-secondary transition-colors font-medium">
                  {c('global', 'nav-inicio')}
                </a>
              </li>
              
              <li>
                <a
                  href={`${basePath}/metodologia`}
                  className="text-gray-600 hover:text-secondary transition-colors font-medium"
                >
                  {c('global', 'nav-metodologia')}
                </a>
              </li>

              <li>
                <a
                  href={`${basePath}/vacation-classes`}
                  className="text-gray-600 hover:text-secondary transition-colors font-medium"
                >
                  {c('global', 'nav-vacation')}
                </a>
              </li>

              {/* <li>
                <a
                  href={`${basePath}/infraestrutura`}
                  className="text-gray-600 hover:text-secondary transition-colors font-medium"
                >
                  Infraestrutura
                </a>
              </li> */}

              <li>
                <a
                  href={`${basePath}/matriculas`}
                  className="text-gray-600 hover:text-secondary transition-colors font-medium"
                >
                  {c('global', 'nav-matriculas')}
                </a>
              </li>
            </ul>
          </div>

          {/* Contato */}
          <div>
            <h3 className="text-primary font-semibold mb-4">{c('global', 'footer-contato-titulo')}</h3>
            <ul className="space-y-3">
              <li className="flex items-center space-x-3">
                <PhoneIcon className="h-5 w-5 text-secondary" />
                <a
                  href="tel:+556236367775"
                  className="text-gray-600 hover:text-secondary transition-colors"
                >
                  {c('global', 'contato-telefone')}
                </a>
              </li>
              <li className="flex items-center space-x-3">
                <EnvelopeIcon className="h-5 w-5 text-secondary" />
                <a 
                  href="mailto:englishpatio@yahoo.com"
                  className="text-gray-600 hover:text-secondary transition-colors"
                >
                  {c('global', 'contato-email')}
                </a>
              </li>
              <li className="flex items-start space-x-3">
                <MapPinIcon className="h-5 w-5 text-secondary flex-shrink-0" />
                <span className="text-gray-600">
                  {c('global', 'footer-endereco-linha1')}<br />
                  {c('global', 'footer-endereco-linha2')}
                </span>
              </li>
              <li className="flex items-start space-x-3 mt-2">
                <svg className="h-5 w-5 text-secondary flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span className="text-gray-600">
                  {c('global', 'footer-horario-linha1')}<br/>{c('global', 'footer-horario-linha2')}
                </span>
              </li>
            </ul>

            {/* Redes sociais */}
            <div className="mt-6">
              <h3 className="text-primary font-semibold mb-4">{c('global', 'footer-redes-titulo')}</h3>
              <div className="flex space-x-2 items-center">
                <a
                  href="https://instagram.com/englishpatio"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-gray-600 hover:text-pink-600 transition-colors flex items-center"
                >
                  <svg 
                    className="h-6 w-6 mr-2" 
                    fill="currentColor" 
                    viewBox="0 0 24 24" 
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                  </svg>
                  {c('global', 'contato-instagram-handle')}
                </a>
              </div>
            </div>
          </div>
        </div>
      </div>

      {/* Copyright */}
      <div className="border-t border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <p className="text-gray-600 text-sm">
              © {new Date().getFullYear()} English Patio. Todos os direitos reservados.
            </p>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer; 