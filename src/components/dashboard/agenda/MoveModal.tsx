...
/* Mover / alocar aluno — só destinos válidos, com confirmação de nível.
   Port 1:1 de openMoverKid/mvPick/mvFilter/mvToggleFull/confirmMoverKid
   (dashboard.html l.2860–3009) e de mvAskFull/confirmFullAlloc (l.2957–2995). */

/* busca do modal: sala, nível, família, horário, dia, teacher, período (l.2879) */
function mvSearchStr(t: Turma): string {
...
interface MoveModalProps {
  sid: number;
  ki: number;
  /* destino pré-selecionado (drop com mudança de nível — l.2647) */
  presel?: number;
  /* busca pré-preenchida (aluno solto num card de sala — l.2629) */
  initialQuery?: string;
  onClose: () => void;
  onAskFull: (tid: number) => void;
}

export function MoveModal({ sid, ki, presel, initialQuery, onClose, onAskFull }: MoveModalProps) {
  const { effectiveUser } = useAuth();
  const { toast, toastErr } = useToast();
  const [sel, setSel] = useState<number | null>(presel ?? null); // 0 = "deixar sem turma"
  ...
  const match = (t: Turma, full?: boolean) => (mvSearchStr(t) + (full ? ' cheia' : '')).includes(query);
  const sameLvlV = sameLvl.filter((t) => match(t));
  const othersV = others.filter((t) => match(t));
  const cheiasV = cheias.filter((t) => match(t, true));
  /* a busca abre a lista de cheias se achar algo nela (l.2948) */
  const showFull = fullOpen || (!!query && cheiasV.length > 0);
  ...
  const confirm = () => {
    ...
    toast(
      t ? `${k.n.split(' ')[0]} agora está em ${turmaShort(t)}!` : `${k.n.split(' ')[0]} voltou para a fila de alocação.`,
    );
    onClose();
  };

  const rowCls =
    'w-full flex items-center gap-2.5 text-left p-2.5 rounded-xl border transition hover:bg-[var(--hover)]';
  ...
  const fullRow = (t: Turma) => {
    const sala = salaById(t.sala)!;
    return (
      <button type="button" key={t.id} onClick={() => onAskFull(t.id)} className={rowCls} style={{ borderColor: 'var(--border)' }}>
        <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: sala.c }} />
        <span className="min-w-0 flex-1">
          <span className="text-sm font-medium block truncate">
            {sala.n.replace(' Room', '')} · {schLabel(t.par)} {t.hora}
  ...
  return (
    <Modal
      title={cur ? 'Mover aluno de turma' : 'Alocar aluno numa turma'}
      onClose={onClose}
      size="max-w-lg"
      footer={
        <>
      ...
      <div className="p-5 space-y-4">
        <div className="flex items-center gap-3 p-3 rounded-xl" style={{ background: 'var(--hover)' }}>
          ...
          <div className="min-w-0">
            ...
            <p
              className={`text-xs ${cur ? 'text-[var(--muted)]' : 'font-semibold'}`}
              style={cur ? undefined : { color: '#B5860B' }}
            >
              {cur ? `hoje em ${turmaShort(cur)} · ${nivelLabel(cur.nivel)}` : 'aguardando turma'}
            </p>
          </div>
        </div>
        ...
        {othersV.length > 0 && (
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-[var(--muted)] mb-1.5">
              Outros níveis{' '}
              <span className="normal-case font-normal">(muda o nível do aluno — confirme com cuidado)</span>
            </p>
            <div className="space-y-1.5 max-h-[200px] overflow-y-auto pr-1">{othersV.map(row)}</div>
          </div>
        )}
        {!sameLvl.length && !others.length && (
          <p className="text-sm text-[var(--muted)]">
            Nenhuma turma com vaga no momento — crie uma turma nova pela Agenda.
          </p>
        )}
        {cur && (
          <button
            ...
            className={rowCls}
            style={{
              borderColor: sel === 0 ? '#2F539A' : 'var(--border)',
              background: sel === 0 ? 'rgba(47,83,154,.06)' : undefined,
            }}
          >
        ...
        {cheias.length > 0 && (
          <div className="pt-1 border-t" style={{ borderColor: 'var(--border)' }}>
            ...
            >
              ...
              <ChevronDown className="w-4 h-4 transition-transform" style={{ transform: showFull ? 'rotate(180deg)' : undefined }} />
            </button>
            {showFull && (
              <div className="space-y-1.5 mt-1 max-h-[180px] overflow-y-auto pr-1">{cheiasV.map(fullRow)}</div>
            )}
          </div>
        )}
        {warnLevel && (
          ...
          >
            ...
            <span>
              Mudança de nível: <b>{nivelLabel(cur!.nivel)}</b> → <b>{nivelLabel(selT!.nivel)}</b>. Confirme se é isso
              mesmo.
            </span>
          </div>
        )}
...
/* turma cheia: alocar abrindo 1 vaga extra — sempre com explicação + confirmação
   (mvAskFull/confirmFullAlloc, l.2957). Máximo 2 extras (cap 9). */
interface AskFullModalProps {
  sid: number;
  ki: number;
  tid: number;
  onBack: () => void;
...
export function AskFullModal({ sid, ki, tid, onBack, onClose }: AskFullModalProps) {
  const { effectiveUser } = useAuth();
  const { toast } = useToast();
  const s = STUDENTS.find((x) => x.id === sid);
  const k = s?.kids[ki];
  const t = turmaById(tid);
  if (!s || !k || !t) return null;

  /* no máximo 2 vagas extras além do padrão de 7 — mais que isso não cabe na sala */
  if (t.cap >= 9) {
    return (
      <Modal title="Limite de vagas extras" onClose={onClose} size="max-w-md">
        <div className="p-5 space-y-4">
          <p className="text-sm">
            A turma <b>{turmaShort(t)}</b> já está com <b>{t.cap} lugares</b> — 2 vagas extras além do padrão de 7.
            Abrir mais uma passaria do que cabe na sala.
          </p>
          <p className="text-sm text-[var(--muted)]">
            Escolha outra turma com vaga ou crie uma turma nova pela Agenda.
          </p>
          <button onClick={onBack} className="w-full h-11 rounded-xl border border-[var(--border)] font-medium text-sm">
            Voltar
          </button>
        </div>
      </Modal>
  ...
  return (
    ...
    >
      <div className="p-5 space-y-3">
        ...
        <p className="text-sm">
          Para colocar <b>{k.n.split(' ')[0]}</b> nessa turma, o limite dela aumenta em 1: de <b>{t.cap}</b> para{' '}
          <b>{t.cap + 1}</b> alunos.
        </p>
        <div
        ...
        >
          <AlertTriangle className="w-4 h-4 shrink-0" />
          <span>
            Use só quando realmente couber mais uma criança na sala. A turma deixa de mostrar "NÃO TEM VAGA" até lotar
            de novo — e dá para voltar o limite depois, editando a turma.
          </span>
        </div>
      </div>
    </Modal>
